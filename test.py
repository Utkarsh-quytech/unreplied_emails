...
async def homepage(gevent: models.GEvent):
    ...
    page = await send_reminder(email, creds)
    return page
...
async def get_unreplied_emails(email, creds):
    unreplied_emails = []
    service = build('gmail', 'v1', credentials=creds, requestBuilder=lambda: Request(timeout=60)) # Increase timeout if necessary

    # Get unreplied incoming emails
    next_page_token = None
    while True:
        threads = await service.users().threads().list(userId='me', q='-is:chats -is:sent -is:draft -in:trash', maxResults=100, pageToken=next_page_token).execute()
        if 'threads' in threads:
            for thread in threads['threads']:
                thread_id = thread['id']
                thread_messages = await service.users().threads().get(userId='me', id=thread_id).execute()
                for message in thread_messages['messages']:
                    message_id = message['id']
                    message_details = await service.users().messages().get(userId='me', id=message_id).execute()
...
async def has_been_replied_to(service, thread_id):
    thread = await service.users().threads().get(userId='me', id=thread_id).execute()

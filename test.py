...
def send_reminder(creds, user_timezone):
    unreplied_emails = get_unreplied_emails(creds, user_timezone)
    if unreplied_emails:
...
def get_unreplied_emails(creds, user_timezone):
    unreplied_emails = []
    ...
    if 'threads' in threads:
        for thread in threads['threads']:
            ...
            for message in thread_messages['messages']:
                ...
                message_date_with_timezone = message_date.replace(tzinfo=timezone('UTC')).astimezone(timezone(user_timezone)).strftime('%Y-%m-%d %H:%M:%S %Z')
